...
function anda(_path, _inimigo){
    ...
    if(_path){
        ...
        easystar.avoidAdditionalPoint((_path[1].x),(_path[1].y));
//        setTimeout(function(){
            
        game.time.events.add(Phaser.Timer.SECOND * 0.5, function(){
            var y = _path[1].y * 32;
            var x = _path[1].x * 32;
//            game.add.tween(_inimigo).to( { y: y }, 100, Phaser.Easing.Linear.None, true);
//            game.add.tween(_inimigo).to( { x: x }, 100, Phaser.Easing.Linear.None, true);
//            tween.onComplete();
            _inimigo.x = x;
            _inimigo.y = y;
