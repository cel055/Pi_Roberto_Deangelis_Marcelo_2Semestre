...
function move(_inimigo, _path){
    ...
    if(_inimigo.tween && _inimigo.tween.isRunning){
        _inimigo.tween.stop();
    }
    
    //var duration = (game.physics.distanceToPointer(_inimigo.sprite, _path) / 200) * 1000;
 
	_inimigo.tween = game.add.tween(_inimigo).to({ x: _path[_pos].x, y: _path[_pos].y }, 500, Phaser.Easing.Linear.None, true);
